...
export class ProductList extends Component {
    ...
    async componentDidMount() {
      ...
      const items = await axios.post('http://192.168.2.104:5000/api/product/list',body)
    ...
    handleDelete = async (item) =>{
      ...
      if(x){
        ...
        this.setState({items})
        // console.log(item.id)
          await axios.post('http://192.168.2.104:5000/api/product/remove',item)
        // try {
        //   
              ...
              <tbody>
                ...
                {items.map(item =>
                  <tr key={item.id}>
                    ...
                    <td><Link to = {`/products/productupdate/${item.title}`}><FontAwesomeIcon className ='icon' icon={faEdit} /></Link></td>
                  </tr>  
                )}
