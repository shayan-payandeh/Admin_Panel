...
async componentDidMount() {
  ...
  const items = await axios.post('http://192.168.1.116:5000/api/shop-category-sell/list',body)
...
handleDelete = async (item) =>{
  ...
  if(x){
    ...
    this.setState({items})
    // console.log(item.id)
      await axios.post('http://192.168.1.116:5000/api/shop-category-sell/remove',item)
    // try {
    //   
...
render() {
  ...
  // const {items} = this.state
    return (
      ...
      <Link className = 'btnLink' to ='/shopping/shopselladd'><button className ='btn btn-success btn-sm btn-add'> <FontAwesomeIcon className ='icon' icon={faPlus} /> </button></Link>
          ...
          <tbody>
            ...
            {items.map(item =>
              <tr key={item.id}>
                ...
                <td><Link to = {`/shopping/shopsellupdate/${item.title}`}><FontAwesomeIcon className ='icon' icon={faEdit} /></Link></td>
              </tr>  
            )}
