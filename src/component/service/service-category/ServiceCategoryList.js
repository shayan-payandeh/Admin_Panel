...
export class ServiceCategorytList extends Component {
    constructor(props) {
        super(props)
    ...
    async componentDidMount() {
      ...
      const items = await axios.post('http://192.168.2.104:5000/api/service-category/list',body)
    ...
    handleDelete = async (item) =>{
      ...
      if(x){
        ...
        this.setState({items})
        // console.log(item.id)
          await axios.post('http://192.168.2.104:5000/api/service-category/remove',item)
        // try {
        //   
    ...
    render() {
      ...
      // const {items} = this.state
        return (
          <React.Fragment>
          {/* <Tab/> */}
          <Link className = 'btnLink' to ='/services/servicecategoryadd'><button className ='btn btn-success btn-sm btn-add'> <FontAwesomeIcon className ='icon' icon={faPlus} /> </button></Link>
              ...
              <tbody>
                ...
                {items.map(item =>
                  <tr key={item.id}>
                    ...
                    <td><Link to = {`/services/servicecategoryupdate/${item.title}`}><FontAwesomeIcon className ='icon' icon={faEdit} /></Link></td>
                  </tr>  
                )}
...
export default ServiceCategorytList
