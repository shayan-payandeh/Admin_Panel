...
export class ProductLevelList extends Component {
    constructor(props) {
        super(props)
    ...
    async componentDidMount() {
      ...
      const items = await axios.post('http://192.168.2.104:5000/api/product-level/list',body)
    ...
    handleDelete = async (item) =>{
      ...
      if(x){
        ...
        this.setState({items})
        // console.log(item.id)
          await axios.post('http://192.168.2.104:5000/api/product-level/remove',item)
        // try {
        //   
    ...
    render() {
      ...
      // const {items} = this.state
        return (
          <React.Fragment>
          {/* <Tab/> */}
          <Link className = 'btnLink' to ='/products/productleveladd'><button className ='btn btn-success btn-sm btn-add'> <FontAwesomeIcon className ='icon' icon={faPlus} /> </button></Link>
              ...
              <tbody>
                ...
                {items.map(item =>
                  <tr key={item.id}>
                    ...
                    <td><Link to = {`/products/productlevel/${item.title}`}><FontAwesomeIcon className ='icon' icon={faEdit} /></Link></td>
                  </tr>  
                )}
...
export default ProductLevelList
